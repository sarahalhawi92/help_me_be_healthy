  ...
  <head>
  	<title>Protein: Fish</title>
  </head>

  <body>
  	<h1>Everything you need to know about fish</h1>
...
// Check connection
  	...
  	$query = "SELECT `recipe_1_name`, `recipe_1_price`, `recipe_1_calories`, `recipe_1_fat`, `recipe_1_cholestrol`, `recipe_1_carbs`, `recipe_1_protein`, `recipe_1_fibre`, `recipe_1_sodium`, `recipe_1_potassium`, `recipe_1_source`
  	          FROM `protein` WHERE `ingredient_name`= 'fish'";
  	...
  	$query = "SELECT `recipe_2_name`, `recipe_2_price`, `recipe_2_calories`,`recipe_2_fat`, `recipe_2_cholestrol`, `recipe_2_carbs`, `recipe_2_protein`, `recipe_2_fibre`, `recipe_2_sodium`, `recipe_2_potassium`,`recipe_2_source` FROM `protein` WHERE `ingredient_name`= 'fish'
  	";
  	$data= mysqli_query($dbc,$query) or die('Query failed: ' . mysql_error());
    ...
    <th>Amount of Potassium</th>
      ...
      echo "<td>" . $row['recipe_2_potassium'] . "</td>";
